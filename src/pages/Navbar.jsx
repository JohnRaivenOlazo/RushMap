...
const Navbar = ({ session, handleLogout, router }) => {
  return (
    <motion.nav
    ...
    >
      ...
      <h1
        className="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-l from-[#0072ff] to-[#00c6ff] align-middle cursor-pointer"
        onClick={() => router.push('/')}
      >
        <img src="/logo.png" alt="RushMap Logo" className="w-6 h-6 inline-block mr-2" />
        RushMap
      </h1>
      <div className="flex items-center gap-5">
        {session && (
          <p className="text-sm font-medium">
            Welcome, <span className="font-bold">{session.user?.user_metadata?.display_name}</span>!
          </p>
        )}
        {session ? (
          <button
